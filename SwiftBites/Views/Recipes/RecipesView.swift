...
struct RecipesView: View {
  ...
  @Query(FetchDescriptor<Recipe>(sortBy: [SortDescriptor(\Recipe.name)]))
  ...
  var filteredRecipes: [Recipe] {
    let recipesPredicate = #Predicate<Recipe> { $0.name.localizedStandardContains(query)
    }
    
    let descriptor = FetchDescriptor<Recipe>(predicate: query.isEmpty ? nil : recipesPredicate, sortBy: [SortDescriptor(\Recipe.name)]
    )
